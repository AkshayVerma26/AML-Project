...
class datagen():
    ...
    def generate_sparse_signal(self, p, struct):
        ...
        for i in range(x_lst.shape[1]):
            ...
            if struct=="structured":
                ...
                indices = self.seed.choice(np.arange(0,self.n), size=self.k, replace=False)
            else:
                warnings.warn("Structure of the data not mentioned.")
            x = np.ones(self.k)
            x_lst[indices,i] = x
        return x_lst
    ...
    def data_gen(self, A, p, sigma_lst, struct="structured"):
        Y = []
        X = self.generate_sparse_signal(p, struct)
        SIGMA = []
...
class plotter():
    ...
    def stem_plot(self, X_test, X_est, sigma_i, struct, model, samp=0):
        ...
        plt.legend(loc='best')
        plt.grid()
        plt.title(f"Signal "+ r"$\sigma$"+f"={sigma_i} | {model} | {struct}")
        # plt.show()
        plt.savefig(os.path.join(os.getcwd(), 'stem_plots', f'{struct}_{sigma_i}_{model}.png'))