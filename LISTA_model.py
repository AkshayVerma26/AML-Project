...
def visu_shrink(sigma, M, numLayers, alpha=0.02, c=0.6):
    thr_visu = sigma*math.sqrt(2*math.log(M))
    thr_lst = [thr_visu*alpha*(c**i) for i in range(numLayers)]
...
class LISTA(nn.Module):
    def __init__(self, m, n, A, L, numIter, device):
        ...
        self.thr = visu_shrink(0.057, m, numIter, 0.13)
...
def LISTA_train(X , Y, D, numEpochs, numLayers, device, learning_rate, sigma):
    ...
    #-------------------*****************-_-----------------
    net.weights_init()
    net.__setter__(visu_shrink(sigma, m, numLayers))
